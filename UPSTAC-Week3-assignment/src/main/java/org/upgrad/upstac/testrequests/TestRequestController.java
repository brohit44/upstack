...
public class TestRequestController {
    ...
    public TestRequest createRequest(@RequestBody CreateTestRequest testRequest) {
        try {
            return testRequestService.createTestRequestFrom(userLoggedInService.getLoggedInUser(), testRequest);
    ...
    public List<TestRequest> requestHistory() {
        try {
            return testRequestService.getHistoryFor(userLoggedInService.getLoggedInUser());
        }  catch (AppException e) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, e.getMessage());
        }
    }
